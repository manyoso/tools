...
## Java include files like jni.h
if [ `hostname -s` = "atreat-thinkpad" ]; then
...
if [ `hostname -s` = "cfp-pop" ]; then
    export JAVA_HOME=/usr/lib/jvm/java-1.7.0-openjdk-1.7.0.85.x86_64

    ## Make a nice bash prompt with git branch if available
    export PS1='\[\033[01;1;37m\]\u@\h:\[\033[01;1;33m\]\w>\[\033[0m\] '
fi
...
## tervela specific setup
export NEBULA=~/dev/alpha
export CFP_SCRIPTS=~/dev/cfp
if [ -e "$CFP_SCRIPTS/setup.sh" ]
then
    source $CFP_SCRIPTS/setup.sh
fi
